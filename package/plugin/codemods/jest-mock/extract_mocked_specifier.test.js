...
const { rewriteMocks } = require("./rewrite-mocks");
const {
  multilineTrim,
...
describe("codemods/jest-mock/extract_mocked_specifier", () => {
  it("leaves code as is if there is only one key", () => {
    expectCodemod(
      ...
      `,
      (path) => rewriteMocks(path, "filename"),
      `
      jest.mock('./origin.js', () => ({
        a: 'a'
      }));
  ...
  it("extracts a separate jest.mock statement for a given specifier", () => {
    expectCodemod(
      ...
      `,
      (path) => rewriteMocks(path, "new/path/origin.js"),
      `
      jest.mock("new/path/origin.js", () => ({
        a: 'a',
        b: 'b'
      }));
      `,
